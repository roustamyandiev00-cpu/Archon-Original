...
import { saveAgentMemory } from "@/components/dashboard/agents/memory";
...
export async function sendAgentChatMessage(input: {
  ...
  if (result.useFallback) {
    return { useFallback: true as const, error: result.error };
  }
  ...
  return {
    ...
    remembered: Boolean(result.reply.remember),
  };
}
