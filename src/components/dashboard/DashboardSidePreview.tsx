...
function colorsFor(name: string) {
  const normalized = name === "Nova" || name === "Lima" || name === "Ela" ? "Lara" : name;
  ...
  return key ? agentColorMap[key] : agentColorMap.Lara;
}
...
const mockLogs: LogRow[] = [
  {
    ...
    agent_name: "Viktor",
    action_type: "status",
    message: "Pipeline bijgewerkt en openstaande taken gesynchroniseerd",
...
export default async function DashboardSidePreview({
  ...
  if (companyId && !isPreview) {
    const [agentName, agentConfig] = await Promise.all([
      userId ? loadUserAgentName(supabase, userId) : Promise.resolve("Lara"),
  ...
  const activeLogs = logs.length > 0 ? logs : isPreview ? mockLogs : [];
  const activeAgents =
    agents.length > 0
      ? agents
      : isPreview
        ? [
            { id: "nova", name: "Lara", role: "AI-metgezel", status: "idle" as const, statusLabel: "Stand-by", pending: 0 },
            { id: "schatter", name: "Viktor", role: "Offertes", status: "actief" as const, statusLabel: "Actief", pending: 0 },
            { id: "facturatie", name: "Nina", role: "Peppol", status: "idle" as const, statusLabel: "Stand-by", pending: 0 },
            { id: "opvolger", name: "Daan", role: "Leads", status: "actief" as const, statusLabel: "Actief", pending: 0 },
          ]
        : [];
    ...
    <div className="flex h-full min-h-0 flex-col space-y-5">
      ...
      <div className="grid grid-cols-2 gap-2 shrink-0">
        {activeAgents.length > 0 ? (
          ...
          <p className="col-span-2 rounded-xl border border-zinc-800 bg-zinc-900 px-3 py-4 text-center text-xs text-zinc-500">
            Agents staan klaar — zodra er activiteit is, zie je hier live status.
          </p>
        )}
      </div>
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-white/[0.03] p-3 space-y-2 scrollbar-thin">
          {activeLogs.length > 0 ? (
            ...
            <p className="py-6 text-center text-xs text-zinc-500">
              Nog geen agent-activiteit — acties verschijnen hier zodra Ela of je crew iets uitvoert.
            </p>
          )}
        </div>
      </div>
