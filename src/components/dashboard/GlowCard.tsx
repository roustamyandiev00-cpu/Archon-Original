  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div
        className={
          subtle
            ? `dashboard-glow-card-inner rounded-2xl border border-white/10 bg-zinc-900/50 ${innerClassName}`
            : `dashboard-glow-card-ring rounded-3xl bg-gradient-to-br from-sky-400/70 via-white/25 to-orange-500/70 p-px shadow-[0_0_60px_-15px_rgba(56,189,248,0.45)]`
        }
      >
        {subtle ? (
          children
          ...
          <div
            className={`dashboard-glow-card-inner rounded-[calc(1.5rem-1px)] bg-zinc-950/90 backdrop-blur-xl ${innerClassName}`}
          >
            {children}
          </div>
        )}
