...
type NovaVoiceAskProps = {
  ...
  stepText?: string;
  variant?: "sky" | "orange";
  /** Dashboard: stuur vraag naar Nova-chat i.p.v. lokaal antwoord. */
...
export default function NovaVoiceAsk({
  ...
  function handleQuestion(text: string) {
    ...
    if (onAskChat) {
      onAskChat(text);
      setAnswer("Ik open Nova-chat met je vraag — even geduld.");
      return;
    }
    ...
    <div className="mt-4 space-y-3 border-t border-white/10 pt-4">
      ...
      {answer && (
        <div className="rounded-xl border border-sky-500/20 bg-sky-500/[0.06] px-3 py-2.5 text-xs leading-relaxed text-sky-100/90">
          <span className="font-medium text-sky-300">Nova:</span> {answer}
        </div>
      )}
    </div>
  );
