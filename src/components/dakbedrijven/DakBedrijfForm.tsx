...
import {
  createDakBedrijf,
  uploadDakBedrijfFotos,
  type DakBedrijfCategorie,
} from "@/app/dakbedrijven/actions";
...
export default function DakBedrijfForm() {
  const router = useRouter();
  const [naam, setNaam] = useState("");
  const [categorie, setCategorie] = useState<DakBedrijfCategorie>("bouwbedrijf");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await createDakBedrijf({
      naam,
      categorie,
      adres,
      postcode,
    ...
    if ("id" in res && res.id) {
      if (fotos.length > 0) {
        ...
        const uploadRes = await uploadDakBedrijfFotos(res.id, fd);
        if ("error" in uploadRes && uploadRes.error) {
          setLoading(false);
          setError(
            `Bedrijf toegevoegd, maar foto's uploaden mislukte: ${uploadRes.error}`,
          );
          return;
        }
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="rounded-2xl border border-white/5 bg-zinc-900/30 p-5 backdrop-blur-sm">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <label className={labelClass}>Naam van de winkel / het bedrijf</label>
            <input
              value={naam}
              onChange={(e) => setNaam(e.target.value)}
              placeholder="bijv. Dakmaterialen Janssens"
          ...
          <div>
            ...
            <select
              value={categorie}
              onChange={(e) => setCategorie(e.target.value as DakBedrijfCategorie)}
              className={inputClass}
            >
          ...
          <div className="sm:col-span-2">
            ...
            <input
              ...
              placeholder="bijv. www.dakmaterialenjanssens.be"
              className={inputClass}
            />
          </div>
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Waar is dit bedrijf goed in? Wat verkopen of bouwen ze?"
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <input
              ...
              list="dakbedrijf-regio-opties"
              placeholder="Selecteer of typ een regio"
              className={inputClass}
            />
            <datalist id="dakbedrijf-regio-opties">
              {REGIOS.map((r) => (
      ...
      <div className="rounded-2xl border border-white/5 bg-zinc-900/30 p-5 backdrop-blur-sm">
        ...
        <div className="space-y-5">
          <div>
            <label className={labelClass}>Foto&apos;s van pand of werk (Optioneel)</label>
            <input
              ref={fileInputRef}
              type="file"
              accept="image/*"
      ...
      {success && (
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-300">
          Bedrijf toegevoegd aan de directory. Bedankt!
        </div>
      )}
