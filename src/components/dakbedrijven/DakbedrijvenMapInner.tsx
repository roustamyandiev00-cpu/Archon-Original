...
import { categorieMeta } from "@/lib/dakbedrijven";
...
export default function DakbedrijvenMapInner({
  bedrijven,
}: {
  bedrijven: MapBedrijf[];
}) {
  ...
  useEffect(() => {
    ...
    const markers = bedrijven.map((b) => {
      const cMeta = categorieMeta(b.categorie);
      const marker = L.marker([b.lat, b.lng], { icon: markerIcon }).addTo(map);
      const adresLijn = [b.adres, b.stad].filter(Boolean).join(", ");
      marker.bindPopup(
        `<div style="font-family:inherit;min-width:140px">
          <strong>${escapeHtml(b.naam)}</strong><br/>
