-- Prevent authenticated users from mutating another user's referral profile.
-- Remove unnecessary anonymous access to authenticated team-management RPCs.
...
begin
  ...
  if p_user_id is null or p_user_id <> v_uid then
    raise exception 'Geen toegang tot dit referral-profiel' using errcode = '42501';
  end if;
...
revoke all on function public.ensure_user_referral(uuid, text, text)
  from public, anon, service_role;
grant execute on function public.ensure_user_referral(uuid, text, text)
  to authenticated;

revoke execute on function public.team_list_members(bigint) from public, anon;
revoke execute on function public.team_add_member(bigint, text, text) from public, anon;
revoke execute on function public.team_update_member(bigint, bigint, text, boolean)
  from public, anon;
...
grant execute on function public.team_update_member(bigint, bigint, text, boolean)
  to authenticated;

comment on function public.ensure_user_referral(uuid, text, text) is
  'Creates or returns referral data only for the currently authenticated user.';
